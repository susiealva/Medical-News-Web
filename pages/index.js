  ...
  if (message.role === "bot" && message.article) {
    ...
    return (
      <div className="space-y-2">
        ...
        {summary ? <div className="whitespace-pre-line">{summary}</div> : null}
        {isSafeHttpUrl(url) ? (
          <a
            href={url}
            target="_blank"
...
export default function Home() {
  ...
  const handleSend = async (e, overrideInput) => {
    ...
    const value = overrideInput !== undefined ? overrideInput : input;
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        const articleMessages = data.results.map((article) => ({
          return (
            <>
              <Head>
              ...
              <div className="min-h-screen w-full bg-gradient-to-br from-blue-50 via-indigo-100 to-white flex items-center justify-center py-8 px-2">
                <div className="w-full max-w-md bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden" style={{ minHeight: "600px", maxHeight: "90vh" }}>
                  ...
                  {messages.length === 1 && (
                    <div className="flex flex-wrap gap-2 px-6 pt-4 pb-2 bg-gradient-to-b from-white/80 to-blue-50 border-b border-blue-100">
                      {promptSuggestions.map((suggestion, idx) => (
                        <button
                          key={idx}
                          type="button"
                          className="px-4 py-2 rounded-full bg-blue-100 hover:bg-blue-200 text-blue-800 text-sm font-medium shadow-sm border border-blue-200 transition"
                  ...
                  {/* Input moderno */}
                  <form ref={formRef} onSubmit={handleSend} className="flex items-center gap-3 px-6 py-5 border-t border-gray-100 bg-white/80 rounded-b-3xl">
                    <textarea
