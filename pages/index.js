...
export default function Home() {
  ...
  return (
    <>
      ...
      <div className="min-h-screen w-full bg-gradient-to-br from-blue-50 via-indigo-100 to-white dark:bg-zinc-900 dark:bg-none flex items-center justify-center py-8 px-2">
        <div className="w-full max-w-7xl h-[80vh] bg-white/90 dark:bg-zinc-900 backdrop-blur-md rounded-3xl shadow-2xl border border-gray-100 dark:border-zinc-800 flex flex-row overflow-hidden" style={{ maxHeight: "92vh" }}>
          ...
          <div className="flex flex-col items-center gap-6 w-72 min-w-[220px] bg-gradient-to-b from-blue-700 to-indigo-600 dark:from-zinc-900 dark:to-zinc-800 p-8 text-white dark:text-zinc-100">
                        {/* Switch modo claro/oscuro: solo un botón circular con ícono */}
                        <button
                          className="self-end mb-2 w-10 h-10 flex items-center justify-center rounded-full border border-white/30 dark:border-zinc-700 bg-white/10 dark:bg-zinc-800/60 shadow hover:bg-white/20 dark:hover:bg-zinc-700 transition text-2xl"
            ...
            <h2 className="text-2xl font-extrabold tracking-tight drop-shadow text-center dark:text-zinc-100">{APP_NAME}</h2>
            ...
            {messages.length === 1 && (
              <div className="flex flex-col gap-2 w-full mt-8">
                {promptSuggestions.map((prompt, idx) => (
                  <button
                    key={idx}
                    className="px-4 py-2 rounded-full bg-blue-100 hover:bg-blue-200 text-blue-800 font-medium shadow transition text-left dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-100"
          ...
          <div className="flex flex-col flex-1 h-full">
            <ul className="flex-1 overflow-y-auto px-16 py-12 space-y-8 bg-gradient-to-b from-white/80 to-blue-50 dark:bg-zinc-900 dark:bg-none" style={{ minHeight: "480px" }}>
              {messages.map((msg, idx) => (
                <li key={idx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} items-end gap-2`}>
                  {msg.role === "bot" ? (
                    <span className="flex items-center justify-center w-10 h-10 rounded-full bg-blue-600 dark:bg-zinc-800 text-white dark:text-zinc-100 shadow-lg border-2 border-white dark:border-zinc-700">
                      <ChatBubbleLeftRightIcon className="w-6 h-6" />
                    </span>
                  ) : (
                    <span className="flex items-center justify-center w-10 h-10 rounded-full bg-indigo-200 dark:bg-zinc-700 text-indigo-700 dark:text-zinc-100 shadow border-2 border-white dark:border-zinc-700">
                  ...
                  <div
                    className={`max-w-[85%] px-7 py-4 rounded-2xl text-lg font-medium shadow-md transition-all duration-200 ${
                      msg.role === "user"
                        ? "bg-indigo-600 text-white rounded-br-3xl dark:bg-zinc-700 dark:text-zinc-100"
                        : "bg-white/90 text-gray-900 rounded-bl-3xl border border-blue-100 dark:bg-zinc-800 dark:text-zinc-100 dark:border-zinc-700"
                    }`}
              ...
              {loading && (
                <li className="flex justify-start items-end gap-2 animate-pulse">
                  ...
                  <div className="px-5 py-3 rounded-2xl bg-white/90 dark:bg-zinc-800 text-gray-900 dark:text-zinc-100 border border-blue-100 dark:border-zinc-700 font-medium shadow-md">
            ...
            <form
              onSubmit={handleSend}
              className="flex items-center gap-4 px-16 py-8 border-t border-gray-100 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900"
            >
              <textarea
                className="flex-1 resize-none border border-blue-200 dark:border-zinc-700 rounded-full outline-none bg-blue-50 dark:bg-zinc-800 text-lg text-gray-900 dark:text-zinc-100 placeholder-gray-400 dark:placeholder-zinc-400 py-4 px-7 min-h-[56px] max-h-40 focus:ring-2 focus:ring-blue-300 dark:focus:ring-zinc-600"
              ...
              <button
                type="submit"
                className="p-4 rounded-full bg-gradient-to-br from-blue-600 to-indigo-500 hover:from-indigo-500 hover:to-blue-600 dark:bg-zinc-700 dark:hover:bg-zinc-600 text-white disabled:opacity-50 shadow-lg transition"
