...
## Uso de la IA
Se utilizó OpenAI GPT para resumir y analizar la relevancia de las noticias. Los prompts clave fueron iterados para obtener resúmenes útiles y evaluaciones contextuales. El proceso incluyó pruebas con diferentes formulaciones de prompt y ajustes en la lógica de recuperación y presentación de resultados.

## Decisiones técnicas
Se eligió un stack moderno: Next.js + Tailwind CSS para el frontend (por su rapidez de desarrollo y despliegue en Vercel) y FastAPI para el backend (por su sencillez y rendimiento, desplegado en Render). Se integró NewsAPI para la obtención de noticias y OpenAI para el procesamiento de lenguaje natural. La arquitectura desacoplada permite escalar y modificar componentes fácilmente.
...
- **Compatibilidad con la nueva API de OpenAI:** Se actualizó el código backend para usar la sintaxis moderna de openai>=1.0.0.
